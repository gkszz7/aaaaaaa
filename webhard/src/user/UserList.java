	...
	public UserList() {
		...
		searchBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(search.getSelectedItem().toString().equals("이름")){
					...
					searchUser = dao.searchUserByUserPhone(searchField.getText());
					listBySearch(searchUser);
				}
			}
		});
		...
		updateBtn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				UserUpdate ma = new UserUpdate(Userid,UserName,UserCompName,UserPhone,UserAddr,UserList.this);
...
//		if((table.getModel().getValueAt(table.getSelectedRow(), 0).toString()) != null){
	...
	public void listBySearch(List<UserDto> search){		
		...
		for (int i = 0; i < search.size(); i++) {
			 String[] socrates = { search.get(i).getUserId(), search.get(i).getUserName(),
						search.get(i).getCompanyName(), search.get(i).getUserPhone(), search.get(i).getUserAddr() };
